...
# Key: Attacker IP | Value: List of request Unix timestamps
IP_REQUEST_LOGS = defaultdict(list)
MAX_REQUESTS_PER_WINDOW = 5  # Max commands allowed
WINDOW_SECONDS = 10  # Sliding time window in seconds
...
def is_rate_limited(ip):
  ...
  IP_REQUEST_LOGS[ip] = [
      t for t in IP_REQUEST_LOGS[ip] if now - t < WINDOW_SECONDS
  ]
...
def log_attack(ip, command, response):
  ...
  with open(LOG_FILE, "a", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(
        [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), ip, command, response]
    )
...
def start_honeypot():
  ...
  while True:
    ...
    conn.sendall(
        b"Welcome to Ubuntu 22.04 LTS (GNU/Linux 5.15.0 x86_64)\r\nadmin@server:~$ "
    )
      ...
      try:
        ...
        if "\n" in buffer or "\r" in buffer:
          ...
          if is_rate_limited(attacker_ip):
            ai_output = (
                "bash: rate limit exceeded: too many requests. Please wait a"
                " moment."
            )
          else:
